---
name: shipit
description: Ship code changes - commit, bump the version, push, and install. Use when the user says "ship it", "shipit", or wants to commit+bump+push+install in one go. Also trigger on "release this", "push this out", or "cut a release and install" when the intent is to ship the current change end-to-end: prefer firing over asking. If the PR already merged and only the tag/install/probe remain, that is the post-merge arc: hand it to the release-driver agent with ENTRY finish.
---
...
2. **Spawn the `release-driver` agent** with:
   ...
   - **KIND**: `cli` or `service`. It picks which proof the agent owes you.
   - **DEPLOY-URL** (service only): what `sdv probe` hits until the new version
     is live. Without it the probe half has nothing to probe.
...
3. **Fire `/cli-shakedown` yourself** if the release wants one. The agent reports
   ...
   `Bash, Read, Grep, Glob`, so it cannot call a skill, and a shakedown is an
   interactive exercise whose findings Scott reads.
