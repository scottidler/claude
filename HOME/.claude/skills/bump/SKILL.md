...
git push origin <branch>     # then pr-open → PR → CI → review → merge

# after the PR merges:
git checkout main && git pull --ff-only origin main
bump --tag-only              # refuses unless HEAD == origin/main, cannot orphan
git push origin vX.Y.Z       # by explicit name, NEVER --tags
```
...
agent, not a fresh release and never a hand-rolled tag. Spawn the agent with
`ENTRY: finish` and the merged PR url, plus REPO, INSTALL, KIND, and either
DEPLOY-URL (a deployed service) or ACCEPTANCE (a CLI). It reads the PR body's
release-intent line to decide:

- `Release: rides this PR (vX.Y.Z)` → the bump rode the PR. `release --finish`
  tags the merged tip, pushes the tag by name, installs, and then proves the
  version is live (`sdv probe` for a service, the installed binary's `--version`
  plus the acceptance commands for a CLI).
- `Release: none - <why>`, or no line at all → the bump did NOT ride. **STOP and
  ask Scott.** The default is to fold the bump into the next feature PR, never a
  bump-only release branch.

The driver returns "installed at vX.Y.Z, shakedown not run". Running
`/cli-shakedown` is the caller's step, not the agent's.
...
finishes the tag, installs, and proves the new version is live before it reports.
...
```bash
release [-m|-M] [--install "<cmd>"|--no-install]   # ungated: ships; gated: bumps the branch, pushes it, stops at "PR creation required"
# gated: run the pr-open line it printed, then the gh pr create line pr-open prints, then:
release --pr <url>                                  # verifies the PR, then pauses for the merge
release --finish                                    # gated: after the PR merges, pulls main, tags, pushes tag by name
```

`release` does NOT open the PR, by design: a `gh pr create` run inside it is a
subprocess, and PreToolUse cannot see a subprocess. It hands back the literal
`pr-open` command and you run it as your own call, where the gates can read it.
