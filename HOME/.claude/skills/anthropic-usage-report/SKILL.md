...
**Posting is outward-facing and not cleanly reversible**: Slack has no edit/delete for a file message via this path. Always `--dry-run` first, confirm the note wording (goes out in Scott's voice, lead with the point, per `rules/voice.md`) and the file set, then post once. Do not fire a "corrected" follow-up; get it right the first time.
