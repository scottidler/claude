...
- [x] **AC1.** Invoking `Skill(handoff)` returns the skill body, not `cannot be used with Skill tool due to disable-model-invocation`.
  ...
  - *Verified at branch tip `f851161` (2026-09-20):* PASS on the no-refusal half, with the name qualified. The invocation that returned the skill body was `Skill(HOME:handoff)`, the worktree-scoped spelling the harness lists while the cwd is this worktree; the unscoped `Skill(handoff)` still reaches the MAIN checkout's copy until this branch lands there. No `disable-model-invocation` refusal in either spelling's frontmatter: `grep -rn 'disable-model-invocation' HOME/.claude/skills/handoff/` returns nothing. The canonical-name half is provable only after the branch merges (panel Mode 2, W4).
- [x] **AC2, scope corrected (panel Mode 2, M3).** `handoff-guard.sh` fires on exactly the committed handoff-opener corpus and on zero of the 20 committed controls, asserted as set equality, not a coverage percentage. The replay input is each frozen row's extracted `trigger` substring, NOT the original prompt: `fires.tsv`'s ids are `transcript-file:lineno` references into `~/.claude/projects`, which `extract.py` deliberately never committed (3.9 GB, not reproducible). The criterion is written against the committed, deterministic data because that is what CI can re-run.
  ...
  - *Recorded at `f851161` and WRONG, superseded below:* `pass=37 fail=0` was cited as proof of set equality. It was not. The panel's Mode 2 audit (M3) established that nothing in CI read the frozen corpus at all: `handoff-guard-test.sh` never opened `fires.tsv`, `counts.json` or `controls.tsv`, and `controls-check.sh` passed 20/0 only by hand because it does not match the `HOME/.claude/hooks/*-test.sh` glob the `test:` task runs. Left here rather than deleted, because a criterion marked passing on evidence that asserted nothing is the exact defect this doc's gate exists to catch.
  - *Verified after the M3 fold (2026-09-20), commit `6ef5f24`:* PASS, and now actually asserted. `handoff-guard-test.sh` -> `pass=145 fail=0` under `otto ci`, carrying three new blocks: the 20 controls asserted silent with a count assert so the list cannot shrink unnoticed; `fires_sha256` recomputed from `fires.tsv` and compared against `counts.json` with a row-count assert (85); and 85 independent set-equality assertions, one per frozen row. Break-the-code proof: breaking the fire-1 regex fails 92 of 145, deleting a corpus row fails both integrity asserts. Known limit, recorded rather than papered over: replaying the `trigger` substring can never hit the hook's bail patterns (leading `<`, code fence, agent-opener prefix), so this assert catches extraction and matching regressions, not bail-logic ones.
- [x] **AC3.** All three sleep bypass classes deny (18 wrapper spellings for the simple-command fixtures, the 14 parseable ones for the loop fixtures), both halves of each cardinality pair behave (30 x 0.5 allows, 60 x 0.5 denies), all five allow-controls pass, and `bash HOME/.claude/hooks/intent-guard-test.sh` ends `fail=0` with a fixture count above today's.
  ...
  - *Recorded at `f851161`:* `pass=332 fail=0`, up from the 187 baseline. Green, but green over a rule carrying four false-positive denies and one half-parsed duration spelling, which the Mode 2 audit found (M1, M2) and `332/0` did not.
  - *Verified after the M1+M2 fold (2026-09-20), commit `a2a72af`:* PASS. `bash HOME/.claude/hooks/intent-guard-test.sh` -> `pass=359 fail=0`, 27 fixtures added in five new sections plus a `runsays` helper that asserts a deny's SENTENCE, not only its decision. All four false positives flip to allow, both fractional spellings (`.5`, `15.`) now deny at 30s where they previously allowed at zero, and each of the five fixes was reverted in a scratch copy to prove its fixtures bite (356/3, 356/3, 357/2, 354/5, 358/1).
...
- [x] **AC5, ceiling amended to 502 bytes (Addendum A.4).** The always-on prefix grows by no more than 502 bytes over its pre-F2 value, on the rules-plus-memory-files basis stated below. The original 400 was a projection, not a measurement; Scott ruled on 2026-09-20 to buy the 102 bytes rather than drop the citation that makes the ToolSearch facts re-verifiable.
  ...
  - *Verified at branch tip `f851161` (2026-09-20):* 64,668 bytes (51,849 in the 14 always-on rules + 12,819 in the four memory files, the command below, and `wc -c` over the four). Equals the amended 64,668 ceiling exactly, so **PASS**; it fails the original 64,566.
  - *Basis command, corrected (panel Mode 2, W1):* a bare `cat HOME/repos/.claude/rules/*.md | wc -c` returns **99,840**, not 51,849, because six of the 20 rule files are path-scoped and are NOT in the always-on prefix. The 14-file basis is reproduced exactly by excluding them: `ls HOME/repos/.claude/rules/*.md | grep -vE '/(python|rust|js-ts|yaml|comments|terraform)\.md$' | xargs cat | wc -c` -> `51849`, over 14 files (`cli general git interaction logging marquee otto pr recall safety search secrets taste voice`). `rust.md` alone is 34,024 bytes, which is why the unfiltered number is nearly double.
  - *Headroom, noted by the panel:* the amended ceiling equals the observed total exactly, so AC5 now passes with **zero headroom**. Any further always-on byte in this chunk fails it, which is the intended property of an amended-to-actual ceiling, not an oversight.
...
- **A.4. AC5's ceiling is amended from 400 to 502 bytes.** The three-fact ToolSearch section landed at +364 against a ~250 projection, which with Phase 2's +138 puts the chunk 102 bytes over the original cap. Scott's call: accept the overage, keep the citation. Grounds: the 400 was a projection made before the section was written, and the 102 bytes are entirely the three named harness usage sites (memory tools, artifact tool, `EndConversation`) that let a future reader re-verify the facts instead of trusting them. A rule that cannot be checked is the failure mode this chunk exists to reduce, and 102 bytes is 0.16% of the 64,166-byte prefix.
  - **To reverse, arithmetic corrected (panel Mode 2, W2):** deleting the citation sentence from `HOME/repos/.claude/rules/general.md:84-88` returns **79 bytes**, which leaves the chunk at +423 and still over the original 400 cap. Only deleting the whole 364-byte section gets under it, and that deletes Phase 6. So the original cap was not reachable by trimming the citation: the choice was always accept the overage or drop the phase, and the earlier "roughly 330 bytes, under the original cap" was wrong. Revisit condition: the always-on prefix is measured as a cost that matters, at which point the whole 64 KB is the target, not this section.
