...
- **Excluded-compound deny, the third rails rule** (OQ3, Scott's option D). `sandbox.excludedCommands` matches any stage ANYWHERE in a compound and exempts the ENTIRE compound, so appending `; ssh -V` to anything opts it out of the sandbox with no approval prompt. Only the model can compose such a command, so rails removes the composition:
  ...
  - Known blast radius, measured 2026-09-13 against the live list and recorded before the commit: a pipe or `&&` beside an excluded stage is a deny, so `otto ci 2>&1 | tail -50`, `cargo test | rg fail` and `git -C p status && cargo test` are all denied and must be split into separate Bash calls. The narrowing on the table if that proves too costly is a read-only consumer set (`tee`, `tail`, `head`, `cat`, `rg`, `grep`, `jq`, `sort`, `uniq`, `wc`, `less`) joining the transparent list, which keeps `; ssh -V` and `| sh` denied. Not taken unilaterally: it is a rule-semantics call, open for Scott.
