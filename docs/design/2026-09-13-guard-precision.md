...
- Six guards source it: `manifest-scope-guard.sh`, `secret-echo-guard.sh`, `branch-pr-title-guard.sh`, `git-release-guard.sh`, plus the new `branch-name-guard.sh` and the converted `git-no-dash-c.sh`.
...
- `git-no-dash-c.sh` converted in place from a deny to an `updatedInput` rewrite. It stays a shell hook and keeps its name.
...
Bash tool call
  -> settings PreToolUse (matcher "Bash")
       ...
       git-no-dash-c.sh      (deny -> updatedInput rewrite)
session start
...
| `git-no-dash-c.sh` | heredoc, subshell, comment, plus `cmdword_is git`, NO optarg | reading the `-C` target IS the job; optarg masking would erase it |
...
"PreToolUse": [
  { "matcher": "Bash", "hooks": [
      ...
      { "type": "command", "command": "~/.claude/hooks/git-no-dash-c.sh" },
...
`git-no-dash-c.sh` keeps its registration and its name. The name still describes the intent (no `-C` on the cwd); renaming it would leave a dead symlink at `~/.claude/hooks/git-no-dash-c.sh` that the manifest link step does not remove, which is the same dead-file class item 1 exists to kill. `branch-name-guard.sh` is added.
...
#### Phase 6: `git-no-dash-c.sh` becomes an `updatedInput` rewrite
**Model:** opus
- Rewrite `HOME/.claude/hooks/git-no-dash-c.sh` around `lib.sh`: per statement, when the command word is `git` and a `-C <arg>` is present whose value matches the payload's `cwd` per the Data Model, emit `updatedInput` carrying every original `tool_input` field with `command` replaced. Otherwise the explicit `echo '{}'` it lacks at `:21` today. The wholesale-replacement comment from `rewrite-cd-read.py:920-926` is copied into this file, because that is the one mistake this shape invites.
- New `git-no-dash-c-test.sh`, wired into `.otto.yml`: `git -C <cwd> status` strips; `git -C "$PWD" status` strips (allowed today); `git  -C  <cwd>  status` with doubled spaces strips (allowed today); `git -C <cwd>/ status` strips (trailing slash); `git -C . status` strips; `git -C <other> status` unchanged; `git -C <cwd> status && git -C <other> log` strips only the first; `-C` inside a quoted string unchanged; `-C` inside a heredoc body unchanged; `git -C <cwd> status` with a sibling `dangerouslyDisableSandbox` field in `tool_input` emits that field unchanged (the scar-tissue case, asserted directly); a command with no `git` unchanged.
...
- **Success criteria:** `git-no-dash-c-test.sh` exits 0, including the `dangerouslyDisableSandbox` passthrough case. A live Bash call `git -C <session cwd> rev-parse --show-toplevel` succeeds with no denial and the tool result carries the reason line naming the strip. The same call inside an Agent behaves identically (per spike 0c).

#### Phase 7: `hooks-preflight.sh` and the CI resolve check
**Model:** sonnet
- New `HOME/.claude/hooks/hooks-preflight.sh` per the Data Model, using the channel Phase 0a proved. Register on `SessionStart`.
...
- [ ] **AC1:** every bash guard that splits statements SOURCES the shared library. `rg -l '\. "\$\(dirname "\$0"\)/lib\.sh"' HOME/.claude/hooks/` lists exactly the six guards `branch-name-guard.sh`, `branch-pr-title-guard.sh`, `git-no-dash-c.sh`, `git-release-guard.sh`, `manifest-scope-guard.sh`, `secret-echo-guard.sh`. `bash HOME/.claude/hooks/lib-test.sh` exits 0.
  - Observed on main (2026-09-13): `lib.sh` does not exist, so the pattern returns nothing (rc=1). `strip_heredocs` exists in exactly one file (`git-release-guard.sh:342`). Passes after Phase 6.
...
- [ ] **AC5:** a Bash call `git -C <session cwd> rev-parse --show-toplevel` is NOT denied and its tool result carries the reason line naming the strip; `git-no-dash-c-test.sh` exits 0 including the case asserting a sibling `dangerouslyDisableSandbox` field survives the `updatedInput` replacement.
