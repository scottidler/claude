...
- [x] **AC1:** every bash guard that splits statements SOURCES the shared library. `rg -l -g '!lib.sh' '\. "\$\(dirname "\$0"\)/lib\.sh"' HOME/.claude/hooks/` lists exactly the five guards `branch-name-guard.sh`, `branch-pr-title-guard.sh`, `git-release-guard.sh`, `manifest-scope-guard.sh`, `secret-echo-guard.sh`. `bash HOME/.claude/hooks/lib-test.sh` exits 0.
  ...
  - Observed after push (2026-09-14 10:10, `origin/main` = `e6b58ee`, hooks linked, run through the `~/.claude/hooks/` paths): the grep lists exactly `branch-name-guard.sh branch-pr-title-guard.sh git-release-guard.sh manifest-scope-guard.sh secret-echo-guard.sh`; `lib-test.sh` `pass=135 fail=0`. PASS.
...
- [x] **AC2:** the two prose false positives return `{}`. Fixture A: a `cat > x.md <<'EOF'` whose body contains the word `manifest`, piped to `manifest-scope-guard.sh`. Fixture B: `echo "RIPGREP_CONFIG_PATH=$RIPGREP_CONFIG_PATH"`, piped to `secret-echo-guard.sh`.
...
- [x] **AC3:** `git-release-guard.sh --self-test` exits 0, and these six flip: `git tag -f -a v1 -m moved` deny; the combined short form `git tag -fa v1 -m moved` deny (the spelling a model actually types); `git tag -a v9.9.9 -m probe` on a non-main branch deny; `git checkout -b chore/bump-0.9.0` deny; `git push origin "$(git describe --tags --abbrev=0)"` allow; `git checkout -- <one tracked file>` on a dirty tree allow; `gh pr create --body-file "$TMPDIR/body.md"` carrying a `Release:` line allow.
  ...
  - Observed after push (2026-09-14 10:10, branch `guard-precision` at the same SHA as main, fixture repo for the dirty-tree and body-file cases): `--self-test` `pass=269 fail=0`; `git tag -f -a v1 -m moved` deny; `git tag -fa v1 -m moved` deny; `git tag -a v9.9.9 -m probe` off main deny; `git checkout -b chore/bump-0.9.0` deny; `git push origin "$(git describe --tags --abbrev=0)"` allow; `git checkout -- tracked.txt` on a dirty tree allow; `gh pr create --body-file "$TMPDIR/body.md"` with a `Release:` line allow. Plus the audit's wrapped shapes: `{ git reset --hard; }`, `eval "git push origin --tags"`, `echo <(git push origin --tags)` all deny. PASS.
- [x] **AC4:** `gh pr create --head fix/markdown-dark-mode --title 'fix(render): markdown dark mode'` denies with text containing `git branch -m`; `git checkout -b fix/x` denies; `git checkout -b fix-auth-bug` allows; `branch-name-guard-test.sh` and `branch-pr-title-guard-test.sh` exit 0.
  ...
  - Observed after push (2026-09-14 10:10): the `gh pr create --head fix/markdown-dark-mode` case denies with text containing `git branch -m fix-markdown-dark-mode`; `git checkout -b fix/x` deny; `git checkout -b fix-auth-bug` allow; `branch-name-guard-test.sh` `pass=97 fail=0`; `branch-pr-title-guard-test.sh` `pass=50 fail=0`. PASS.
- [x] **AC5:** a Bash call `git -C <session cwd> rev-parse --show-toplevel` is NOT denied, prints the toplevel, and `~/.cache/claude/rewrite-cd-read.log` gains a `STRIP-C` line for it; `git-no-dash-c.sh` is absent from `HOME/.claude/hooks/` and from `settings.json`; `rewrite-cd-read-test.sh` exits 0 including the case asserting a sibling `dangerouslyDisableSandbox` field survives the `updatedInput` replacement.
  ...
  - Observed after push (2026-09-14 10:10): the live main-thread call `git -C /home/saidler/repos/scottidler/claude rev-parse --show-toplevel` printed the toplevel with no denial and the log gained `STRIP-C flag-pointed-at-cwd` (9 such lines in the live log by this run, from this session and its subagents); `git-no-dash-c.sh` absent from `HOME/.claude/hooks/` and from `settings.json`; `rewrite-cd-read-test.sh` `pass=33 fail=0` including the `dangerouslyDisableSandbox` passthrough. PASS.
- [x] **AC6:** `bin/hooks-resolve` exits 0 against the repo and non-zero against a fixture settings file naming a missing hook; a new session started with that fixture surfaces a warning naming the hook.
  ...
  - Observed after push (2026-09-14 10:10): `bin/hooks-resolve` rc=0 against the repo, rc=1 against a fixture naming `~/.claude/hooks/does-not-exist.sh`; the live preflight prints nothing (every registered hook resolves through `~/.claude/hooks/`); against the fixture it emits `hooks-preflight: unresolved hook(s): ~/.claude/hooks/does-not-exist.sh; fix: ...`. The interactive half ran earlier the same day under tmux with `HOOKS_PREFLIGHT_SETTINGS` on that fixture: the fresh session quoted `SessionStart hook additional context: hooks-preflight: unresolved hook(s): ~/.claude/hooks/does-not-exist.sh` verbatim. PASS.
- [x] **AC7:** `awk 'FNR==1 && $0 != "---" {print FILENAME}' HOME/repos/.claude/rules/*.md` prints `voice.md` and nothing else, and each of the three stale release-flow sites stops claiming a tag is created first, checked per file against its OWN wording: `rg -c 'tags local HEAD' HOME/repos/.claude/rules/git.md HOME/.claude/hooks/git-release-guard.sh` prints nothing, AND `rg -c 'tags HEAD' HOME/.claude/agents/release-driver.md` prints nothing.
  ...
  - Observed after push (2026-09-14 10:10): the awk scan prints `voice.md` only; `rg -c 'tags local HEAD'` over `git.md` and the hook prints nothing; `rg -c 'tags HEAD'` over `release-driver.md` prints nothing. PASS.
...
- After merge on desk.lan the operator step is now two commands: `manifest -l HOME/.claude/hooks/* | bash` from the repo root (glob unquoted) to link `lib.sh`, `branch-name-guard.sh` and `hooks-preflight.sh`, then `rm ~/.claude/hooks/git-no-dash-c.sh` to drop the symlink Phase 6 leaves dangling.
  - **Build-time finding (2026-09-14, after Phase 7):** the link step cannot wait for the merge when the phases run in a live session. Phases 2 to 5 ported four guards onto `lib.sh`, whose symlink did not exist until the step ran, so through the symlink path every ported guard hit its defensive source and returned `{}`: the release guard allowed `git tag -fa v1 -m moved` live while its 113-case matrix was green. The fail-open design did exactly what decision 8 and the Rollout Plan say it should, and the Phase 7 preflight run against the live file was what surfaced it (it named `branch-name-guard.sh`, `hooks-preflight.sh` and `lib.sh`). The step ran at that point, scoped as `manifest -l HOME/.claude/hooks/*.sh HOME/.claude/hooks/*.py | bash` because the bare `*` also emits a link for `__pycache__/*.pyc`. Rule for later chunks: a phase that adds a file another live hook depends on runs the link step in the SAME phase, before its live check.
  - `rkvr rmrf` cannot remove the dangling `git-no-dash-c.sh` link: it follows the link and fails `No such file or directory` (rkvr v0.1.22, `src/main.rs:468`). The link is unregistered and harmless; removing it is Scott's plain `rm`, or an rkvr fix for dangling symlinks.
