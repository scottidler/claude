...
Empty through the build. These three were opened by the round-1 implementation audit, after the phases landed. None blocks chunk C, which is Implemented.

- [ ] **Step 3's "ONE foreground Bash call" is un-followable and needs an owner.** Audit must-fix 3. Attempted 2026-09-14 and STOPPED, deliberately: the obvious fix is a single `panel-dispatch.sh` that holds the compound internally and goes into `sandbox.excludedCommands`, which the auto-mode classifier refused as a security weakening, correctly. An excluded head whose body is an arbitrary compound is a general escape hatch, not a targeted fix: today it wraps two seat scripts, and nothing structural stops it wrapping anything. The non-weakening alternative is to teach the rails `tool.call` hook that a compound whose non-excluded stages are inert (`wait`, `echo`, `wc`, `tee` writing only inside the run dir) is permitted, which is narrower but is chunk A or I territory and needs its own doc. `review-panel.md` Step 3 now carries a paragraph telling the agent to report the deny rather than silently detach, which is the safe interim. The rails `tool.call` hook denies a Bash call that combines a `sandbox.excludedCommands` head with another statement, and both seat scripts are in `excludedCommands`, so the launches and the `wait` cannot share one call. Every panel run from now on either splits the seats (losing the PID-namespace property from the 2026-08-31 detached-seat incident) or is denied. Chunk C rewrote this file but the conflict is chunk A's `excludedCommands` against the agent's mandate, so the fix belongs to whichever chunk owns that overlap, not to C.
- [ ] **The remaining run-dir artifacts still disagree with Step 0.1's `-r$ROUND` rule.** `doc-snapshot.md`, `prompt.txt`, `arch.out`, `staff.out` and `staff-sub.txt` are all unsuffixed. Phase 3 fixed only `dispatch-status.txt`, because only that one was wired to Step 5's exit contract. No longer latent: the round-1 audit tripped over it while reviewing, its own artifacts being `-r1`-suffixed while Step 3.75 tells it to read `arch.out`. Measured friction, not untidiness.
- [ ] **`spec-review`'s independent "Max 3 rounds" is now the odd one out.** Already flagged as a Non-Goal before the build (`HOME/.claude/skills/spec-review/SKILL.md:13,213`). Recorded here too because chunk C shipping a mechanical cap is what makes that prose cap read as inconsistent.
