...
- [ ] **The remaining run-dir artifacts still disagree with Step 0.1's `-r$ROUND` rule.** `doc-snapshot.md`, `prompt.txt`, `arch.out`, `staff.out` and `staff-sub.txt` are all unsuffixed. Phase 3 fixed only `dispatch-status.txt`, because only that one was wired to Step 5's exit contract. The rest are the same latent inconsistency with nothing currently depending on them.
