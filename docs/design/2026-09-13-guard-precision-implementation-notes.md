...
## Audit fold-in (round 1)

Round-1 implementation audit (`docs/design/2026-09-13-guard-precision-audit/`):
four must-fix findings and six cheap wins, folded into one commit on the same
branch. Not a phase: no new spec, no new scope, the same files Phases 1 to 8
wrote.

### Design decisions
- MF1, half one, statement boundaries. `boundary_at` in `lib.sh` replaces the inline separator test in `split_one`: unquoted `(`, `)`, and a `{`/`}` that is its own word now end a statement the same way `;` does, so `(git push origin --tags)`, `{ git reset --hard; }`, `case x in x) git push origin --tags;; esac` and `f(){ git push origin --tags; }; f` all hand the inner command to a gate. Two exclusions keep it honest: `subspan` marks the delimiters of `$(`, `<(` and `>(` in `nd[]` so those parens are not compound-command parens, and a brace counts only when whitespace-delimited, which is bash's own rule and what leaves `${x:---tags}` and `xargs -I {}` whole.
- MF1, half two, the command word. `cmdword_ok` no longer regex-matches a statement prefix; `cmdword_index` walks the token map and steps over exactly what the shell steps over: env assignments, the reserved words in `RESERVED`, and the wrappers in `WRAPPERS` with their own options (and the option VALUE for the flags in `OPTVALFLAGS`), plus `timeout`'s duration operand. The three tables live in the awk `BEGIN` block with a comment saying they are tables and how to extend them. Quote removal on the verb comes free: `tokword` already drops quote characters, so `"git"` and `g""it` are `git`, and a leading `\` and a path prefix come off in `verbword`.
- `eval` is a nested statement, not a wrapper word. `find_eval` yields the argument (quote characters dropped, since the shell removes them before parsing what is left) and neutralizes the span in the enclosing statement, the same treatment `bash -c` gets. That covers both `eval "git push origin --tags"` and the unquoted `eval git push origin --tags`. The extent is the rest of the simple command, computed with `eval_end` off the same boundary test, so a following `; other` is not swallowed.
- `unquote` (new): deletes the quote CHARACTERS and keeps their contents, which is what the shell does before git ever sees the word. `git-release-guard.sh` computes `mu` once per statement and every gate MATCHES on it; extraction stays on `$m`/`$s` because unquote is not length preserving and merges words. `git push origin --ta""gs` and `-"-tags"` now deny, and `git push origin "--tags"` still denies, so the Data Model's "option detection never masks quotes" rule is intact: this is removal, not masking.
- MF2: `scan` treats `<(` and `>(` exactly like `$(` (class P, recorded as a nested statement, neutralized in the enclosing one), because bash runs a process-substitution body.
- MF3: `is_dashc` matches any combined short-flag cluster containing `c` (`-lc`, `-xc`, `-lec`), and both `find_dashc` and `dashc_before` call it, so the class map and the nested-statement yield agree on what a `-c` argument is.
- MF4: an unquoted-delimiter heredoc body gets its own byte class `h`, distinct from a quoted-delimiter body and any terminator line (`H`). Default behavior is unchanged: `mask_heredoc` masks `Hh` and `emit_piece` neutralizes both, because a heredoc body is never a command. `heredoc_expanded` emits only the `h` spans, and `secret-echo-guard.sh` takes it as a second input with its own rule: a secret name inside a body bash substitutes into is a leak whatever command consumes it, so `cat <<EOF` / `$GH_TOKEN` / `EOF` denies while `cat <<'EOF'` still allows.
- `manifest-scope-guard.sh` reads its command word from a copy masked with heredoc and comment only, NOT the quote maskers, so `"manifest"` is the manifest command. Nothing is lost: what keeps `echo "=== manifest entry ==="` out is the anchor (the command word is `echo`), not the masking. The quote-masked copy stays the input to the scope-flag and exemption matches, where a quoted flag is not a flag.
- CW3: the attached short-flag spellings (`git checkout -bfix/x`, `-Bname`, `git switch -cname`, `-Cname`, `git worktree add -bname`) are read by both branch-name extractors.
- CW4: `hooks-preflight.sh`'s remediation string now names the two explicit globs instead of the bare `hooks/*` the notes already recorded as wrong.
- CW5: the `.otto.yml` lint list gained the design doc, the implementation notes, the Phase 0 evidence file, both audit files, and `shapes.sh`.
- CW6: `branch-name-guard-test.sh` and `branch-pr-title-guard-test.sh` are executable, matching their 26 siblings.
- Tests, the structural half. Every REGRESSION and bypass row from `probes.md` is a fixture with its exact command text, in the matrix it belongs to. On top of that, `shapes.sh` holds ONE list of 18 spellings that run the same command (the 16 wrapper templates plus the two verb mutations, `\verb` and `"verb"`), and each of the four guard matrices re-runs its irreversible-or-leak deny fixtures through all of them: 126 in the release matrix (tag delete, tag force, `push --tags`, force-push main, `reset --hard`, `clean -fd`, tree-wide `checkout -- .`), 36 in the secret matrix, 36 in the manifest matrix, 36 in the branch-name matrix. 234 wrapped assertions from 13 fixtures, and a new shape is added in one place.
- Matrix counts: lib 93 to 135, release 113 to 269, secret 30 to 73, manifest 23 to 61, branch-name 52 to 97; title 50, preflight 8, rewrite-cd-read 33 unchanged. Every pre-existing expectation kept its verdict.
- Break-the-code evidence, one revert at a time, `lib.sh` restored from a copy each time (no `git checkout`): `cmdword_index` returning its start index fails 55 release, 14 manifest and 13 branch-name cases; parens and braces not being boundaries fails 25, 6 and 6; `<(`/`>(` not scanned fails the 2 `lib-test.sh` yielding cases (the guard matrices stay green because the paren boundary catches that shape second, which is why MF2's proof lives in the library matrix); `is_dashc` back to the exact token fails 5 secret and 7 release cases; every heredoc body inert again fails the 2 expanded-body secret cases.

### Deviations
- The shared test file is `shapes.sh`, not the suggested `test-lib.sh`: `lib-test.sh` already exists in the same directory and means the opposite thing, and the two names are one hyphen apart.
- `cmdword_index` is word-agnostic: it resolves the verb and the caller compares, with no early return when a wrapper word IS the word asked about. So `cmdword_is env` on `env X=1 git push` answers `git`. That is the truthful answer, and no guard asks about a wrapper word.
- No separate "case pattern token ending in `)`" skip rule. The `)` boundary already ends the statement before the pattern can sit in front of a verb, so the rule would be dead code.
- `heredoc_expanded` is a new mode and function rather than an expand-mode toggle on `mask_heredoc`/`stmts`. A toggle that changed `stmts` output would fork the statement-index space `cd_at` walks, and the secret guard needs the bodies as a separate input to a distinct matcher rule anyway (its `echo|printf` rule cannot fire on `cat <<EOF`). The awk program is not copied; this is one more mode on it.
- Two rows `probes.md` marks SAME (pre-existing) flip to deny: `(manifest apply)` and `(git checkout -b feat/thing)`. They are the same class MF1 names, they were bypasses on both sides of the baseline, and leaving them allowed would mean the paren boundary works everywhere except two guards.
- CW3's attached-flag handling also went into `git-release-guard.sh`'s Gate C extractor, which the audit did not name. It is the same extractor with the same hole, and sibling gates behaving differently is its own defect.
- The release guard matches EVERY gate on the unquoted copy, not only the option gates. A quoted verb defeats the subcommand regexes (`\bgit[[:space:]]+tag`) exactly as it defeats a flag match, and the wrapper sweep asserts the quoted-verb spelling against all seven irreversible fixtures.
- The four guard matrices now resolve their own directory with `readlink -f "$0"` (the pattern Phase 9 already applied to two others) and source `shapes.sh` fatally, because the first version of the sweep printed "command not found" 126 times and still exited 0.

### Tradeoffs
- `unquote` is deliberately outside the offset-preserving contract: it is shorter than its input and can merge two words into one. Matching only, never extraction, stated at the definition and in the file header. The alternative, a length-preserving "drop the quotes but keep the geometry" pass, cannot exist: removing a byte is what makes the flag match.
- The secret guard denies a secret name in ANY expanded heredoc body, not only one that reaches the transcript. `wc -l <<EOF` with a secret in the body is a deny it does not strictly need. Fail closed: the value is already substituted before any command sees it, and a body that reaches a file is a secret written to disk, which the secrets rule forbids anyway.
- `dashc_before` widened along with `find_dashc`, so a single-quoted argument after any `-*c*` cluster keeps class `c` (not masked by `mask_squote`). That direction is fail-closed (more visible text, not less) and it keeps one definition of what a `-c` argument is.
- Making `(` and `)` boundaries splits a function definition into `f` and its body, so `stmts` reports three statements for `f(){ ls; }; f`. The statement INDEX space is what `cd_at` walks, so this shifts indices for commands containing parens. Every `cd_at` case in the matrices still passes, and the alternative (parsing function definitions properly) buys nothing a gate reads.

### Open questions
- `g=git; $g push origin --tags` still allows. The verb arrives through a variable, so no matcher that does not RUN the command can know what it is; the audit reached the same verdict. Pinned as an allow fixture in the release matrix so it is a decision and not a surprise.
